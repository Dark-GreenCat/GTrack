...
int main(void)
{
  ...
  uint8_t shadow_dis = 0;
  uint8_t data = 4;
  uint8_t shadow_dis_u8 = 123;

  // while(1) {
  //   HAL_Delay(1000);
  //   HCL_BMA253_Write_Reg(&pcl_bma253.bma253, 0x10, &data, 1);
  //   APP_UART_OutChar(huart_terminal, '\n');
  //   HCL_BMA253_Read_Reg(&pcl_bma253.bma253, 0x10, &shadow_dis_u8, 1);
  //   APP_UART_OutNumber(huart_terminal, shadow_dis_u8);
  //   //HCL_BMA253_set_shadow_dis(&pcl_bma253.bma253, shadow_dis);
  //   //shadow_dis = shadow_dis? 0 : 1;
  // }
  while (1) {
    /* USER CODE END WHILE */
